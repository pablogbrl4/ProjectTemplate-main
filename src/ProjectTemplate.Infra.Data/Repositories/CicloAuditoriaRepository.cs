...
namespace Orizon.Rest.Chat.Infra.Data.Repositories
{
    ...
    public class CicloAuditoriaRepository : BaseRepositorio, ICicloAuditoriaRepository
    {
        public CicloAuditoriaRepository(PrefatDbContext prefatDbContext, DativaDbContext dativaDbContext)
        ...
        private readonly string cicloDadosApontamentoQuery =
            $@"
              ...
              FROM
                  CONTA C
                  INNER JOIN ITEM I ON I.ID_CONTA = C.ID_CONTA
                  OUTER APPLY (
                      SELECT
                          TOP 1 ITEM_DETALHE.QUANTIDADE,
                          ITEM_DETALHE.VALOR_UNIDADE,
                          ITEM_DETALHE.OBSERVACAO
                      FROM
                          ITEM_DETALHE
                      WHERE
                          ITEM_DETALHE.FK_ITEM = I.ID_ITEM
                      ORDER BY
                          ID_ITEM_DETALHE DESC
                  ) D
              WHERE
                  I.ID_ITEM = @idItem
            ";
